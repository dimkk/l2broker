...
local stores = "e:\\projects\\l2broker\\stores\\";
local cpg = {
	["00"] = "Logout",
	["01"] = "AttackRequest",
...
--function OnOutgoingPacket(p)
--	local id = string.format("%02X",p:GetID());
	-- GetSubId for 0xD0 packets
-- 	if (p:GetID() == 0xD0) and (p:Size() >= 1) then
-- 		p:SetOffset(1);
-- 		id = id .. string.format("%02X", p:ReadUInt(1));
-- 	end;
-- 	PacketLogger("Client", p, id, cpg[id]);
-- end;
...
function parsePackets(p, name, location)
	ShowToClient("parse", "===============new");
	local i, output, size = 0, "", p:Size();
	if (size > 0) then
		local fh = io.open(stores .. name .. ".store", "w+");
		
		p:SetOffset(sellPattern.itemsCountStart);
		count = p:ReadUInt(sellPattern.itemsCountLength);
		ShowToClient("count", count.." ");
		output = output..string.format("%s,%s,%s,%s\n", location.X, location.Y,  location.Z, GetZoneID(location));
		while i < count do
		
			p:SetOffset(sellPattern.itemIdStart + (i * sellPattern.itemLength));
			itemId = p:ReadUInt(sellPattern.itemIdLength);
			--itemName = GetItemName(itemId);
			
			
			p:SetOffset(sellPattern.itemCount + (i * sellPattern.itemLength));
			itemCount = p:ReadUInt(sellPattern.itemCountLength);
			
			p:SetOffset(sellPattern.itemPrice + (i * sellPattern.itemLength));
			itemPrice = p:ReadUInt(sellPattern.itemPriceLength);
			
			ShowToClient("itemId", itemId.." "..sellPattern.itemIdStart + (i * sellPattern.itemLength));
			--ShowToClient("itemName", itemName.." ");
			ShowToClient("itemCount", itemCount.." "..sellPattern.itemCount + (i * sellPattern.itemLength));
			ShowToClient("itemPrice", itemPrice.." "..sellPattern.itemPrice + (i * sellPattern.itemLength));
...
function OnIncomingPacket(p)
		...
		--ShowToClient("test", id);
		if id == "A1" then --if string.match(id, "PrivateStoreListSell") or string.match(id, "PrivateStoreListBuy") then
			PacketLogger("Server", p, id, spg[id]);
			seller = GetTarget();
			sellerName = seller:GetName();
			location = seller:GetLocation();
			parsePackets(p, sellerName, location);
		end;
	end;

function OnCreate()

	this:RegisterCommand("gogo", CommandChatType.CHAT_CMD, CommandAccessLevel.ACCESS_ME);
	function OnCommand_gogo(vCommandChatType, vNick, vCommandParam)
ShowToClient("oncmd", "here");
